...
def test_bm25_retrieval():
    ...
    # 2. Side-by-Side Comparison on the 4 Original Sanity Queries
    ...
    sample_queries = [
        eval_data[0],  # Single-core conduit wiring (IS-ELEC-001)
        eval_data[4],  # LSZH metro cables (IS-ELEC-007)
        eval_data[7],  # OPC 43 cement (IS-CEM-001)
        eval_data[12], # ERW fire hydrant pipe (IS-STEEL-001)
    ]
    ...
    # Specifically test queries targeting BM25 capabilities (exact standard numbers & rare technical terms)
    bm25_targeted_cases = [c for c in eval_data if c.get("category") == "hard_identifier"]
    assert len(bm25_targeted_cases) >= 3, "Expected at least 3 BM25-targeted test queries in eval_set.json"
    ...
    for i, case in enumerate(bm25_targeted_cases, 1):
        ...
        print(f"  Target: [{target_id}] {target_std.number} - {target_std.title[:50]}...")
        ...
        dense_rank = dense_ids.index(target_id) + 1 if target_id in dense_ids else ">5"
        
        print(f"  --> BM25 Rank: #{bm25_rank} (Score: {bm25_results[bm25_rank-1][1]:.2f}) [PASS]")
        print(f"  --> Dense Rank: #{dense_rank} (Score: {dense_results[dense_rank-1][1]:.4f} if dense_rank != '>5' else 'N/A')")
        print(f"  Top-3 BM25 Matches:")
        for r_idx, (ret_id, score) in enumerate(bm25_results[:3], 1):
            r_std = get_standard_by_id(ret_id)
            print(f"     #{r_idx}: [{ret_id}] {r_std.number} (score: {score:.2f})")
