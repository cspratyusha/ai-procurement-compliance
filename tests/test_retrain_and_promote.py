...
# Data files live under the standards_retrieval package
_SR_DATA = _PROJECT_ROOT / "app" / "services" / "standards_retrieval" / "data"
_INTERACTION_LOGS_PATH = _SR_DATA / "interaction_logs.jsonl"
_EVAL_SET_PATH = _SR_DATA / "eval_set.json"
...
class TestRetrainAndPromote(unittest.TestCase):
    ...
    def test_insufficient_data_abort(self):
        ...
        # Use real std_* IDs that exist in the corpus so logs_to_training_data
        # can properly resolve them and count distinct queries
        tiny_logs = [
            InteractionLog(
                query="structural steel for general construction",
                candidates_shown=[{"id": "std_001", "final_score": 0.9, "rank": 1}],
                chosen_id="std_001",
                action="accept",
            ...
            InteractionLog(
                query="PVC insulated cables for electrical wiring",
                candidates_shown=[{"id": "std_007", "final_score": 0.8, "rank": 1}],
                chosen_id="std_007",
                action="accept",
                timestamp="2026-09-19T20:00:00Z",
                source="synthetic"
    ...
    def test_real_end_to_end_cycle_execution(self):
        ...
        # Copy existing live model to sandbox as current model so we don't mutate the live model
        live_model_path = _SR_DATA.parent / "models" / "ltr_model.txt"
