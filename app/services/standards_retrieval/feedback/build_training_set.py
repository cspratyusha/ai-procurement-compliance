...
def main():
    ...
    if not passed:
        ...
        print("!" * 75)
        print(f"Expected Line Count : {expected_lines}, Actual Line Count : {count}")
        if leaked:
        ...
        assert count_recheck == expected_lines, f"Expected {expected_lines} lines after clean regeneration, got {count_recheck}"
        assert len(leaked_recheck) == 0, f"Expected 0 leaked queries, got {leaked_recheck}"
        print(f"[Recovery Action] SUCCESS: Exactly {expected_lines} clean interaction logs restored with 0 eval leakage.")
        print("!" * 75 + "\n")
    else:
        print(f"[Pre-Verification] PASSED: Exactly {expected_lines} logs verified, 0 queries match eval_set.json, all match train_queries.json.")
