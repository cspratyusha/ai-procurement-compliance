...
def test_disagreement_cases():
    ...
    # Test cases that highlight different strengths across difficulty categories
    # Case A: Exact standard number query where BM25 has distinct lexical signal
    case_num = next(c for c in eval_data if c.get("difficulty") == "exact_identifier")
    # Case B: Complex natural language description
    case_desc = next(c for c in eval_data if c.get("difficulty") == "technical_description")
    # Case C: Supersession disambiguation query
    case_sup = next(c for c in eval_data if c.get("difficulty") == "supersession_disambiguation")

    test_cases = [case_num, case_desc, case_sup]
