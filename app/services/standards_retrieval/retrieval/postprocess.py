...
def apply_supersession_penalty(
    ...
    # Check superseded_by_id directly first; fallback to regex-based family extraction
    for item in parsed_items:
        std = item["standard"]
        score = item["score"]
        if std and getattr(std, "status", "").lower() == "superseded":
            leader = None
            # Direct check: superseded_by_id relationship
            target_id = getattr(std, "superseded_by_id", None)
            if target_id:
                succ_item = next((it for it in parsed_items if it["id"] == target_id), None)
                if succ_item:
                    leader = {
                        "id": succ_item["id"],
                        "number": getattr(succ_item["standard"], "number", ""),
                        "score": succ_item["score"]
                    }
            # Fallback: regex family match if superseded_by_id is null or not found in candidates
            if not leader:
                family = extract_base_standard_family(std.number)
